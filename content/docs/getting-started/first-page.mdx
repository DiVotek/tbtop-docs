...
`/admin` redirects to the first registered page with a static path. Discovered pages register in alphabetical class-name order, so with discovery alone the panel root would go to `/admin/login` (`LoginPage`) or, later, `/admin/posts/new` (`PostCreatePage`). List the page you want users to land on first in the panel's `pages()`:
