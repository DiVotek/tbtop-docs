...
<Callout type="warn" title="Discovery alone picks the landing page alphabetically">
  `/admin` redirects to the first registered page with a static path. With discovery only, that is the alphabetically first class name, which is rarely the page you want. A `LoginPage` sorts ahead of `PostsPage`, so signed-in users would land on the sign-in form. Once you have a landing page, list it first in `pages()` next to `discoverPages()`. See [Identity and routing](#identity-and-routing).
</Callout>
