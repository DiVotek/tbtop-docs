...
These settings change the chrome around every page. None of them is required. Colours, fonts, the logo and the rest of the admin's look are covered in [Customize the panel](/docs/getting-started/customization).
...
- **Theme.** `defaultThemeMode()` applies only when the visitor has no saved `tbtop_theme` cookie. If you pick a default other than `system`, change the inline script in `admin.blade.php` too; see [Dark mode](/docs/getting-started/customization#dark-mode). Colours, radius and fonts are not panel settings: they are CSS variables in `resources/css/admin.css`, covered in [Colours](/docs/getting-started/customization#colours).
...
The subclass overrides `headerItems()`, `sidebarItems()` or `footer()` and returns ordinary DSL nodes. [Header, sidebar and footer](/docs/getting-started/customization#header-sidebar-and-footer) has a full example and the list of shell blocks.
