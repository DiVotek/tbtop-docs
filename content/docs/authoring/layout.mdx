...
Both take the same shape as `cols`: an int from 1 to 8 or a breakpoint map. Values outside 1-8 or unknown breakpoints throw.
...
- The form, the table and the list are independent: the form's submit does not reload the table. Add `->refreshTable('orders')` to the effects when it should.
