...
The command creates `app/Admin/Pages/ReportsPage.php` (it appends the `Page` suffix) with a `nav()` entry in the `Main` group. `--path=` overrides the URI, `--group=` sets the nav group, `--no-nav` leaves `nav()` out and `--force` overwrites an existing file. It then reports whether a panel discovers that directory or whether you must add the class to `pages()` yourself. Registration and discovery are covered in [Panel](/docs/getting-started/panel).
