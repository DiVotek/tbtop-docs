---
title: "Pages"
description: "How a Page class maps to a route, how view() builds its tree with the S builder, how one page composes any number of forms, tables and display blocks, and how a CRUD resource becomes a family of pages."
---
...
- **Display** — `displayText`, `displayAlert`, `displayValue`, `displayKeyValue`, `displayImage`, `displayHtml`, `markdown`, `displayDivider`.
...
- **Spans go on a wrapper.** Forms and tables have no `colSpan` of their own. Put them in a `section`, `stack` or `grid` and set `colSpan`/`colStart` on that, as the Products section does.

Tabs work the same way: each tab of `$s->tabs()` can hold its own form or table.

See it in the demo: the [posts index](https://demo.tbtop.dev/admin/posts) has two tables on one page, plus forms inside its create and edit actions, and the [dashboard](https://demo.tbtop.dev/admin/dashboard) mixes stats, charts and display blocks.
