...
Icon names are Lucide names in kebab case: `file-text`, `shopping-cart`, `circle-check`. Every Lucide icon is available without registration. To add a name of your own — an alias, or an icon component with the Lucide component type — register it in the admin client entry and then refer to that name from PHP:

```ts
import { registerIcon } from "@tbtop/inertia-admin";
import { Rocket } from "lucide-react";

registerIcon("launch", Rocket);
```
