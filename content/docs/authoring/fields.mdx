...
**Validation.** `required()` marks the field and adds the `required` rule; `rules()` appends any Laravel rules. The fluent helpers — `nullable()`, `minLength()`, `maxLength()`, `minValue()`, `maxValue()`, `between()`, `integer()`, `unique()`, `exists()` and others — append the same rule `rules()` would. A field only offers the helpers that fit its kind, so a `boolean` has no `maxLength()`. `->regex()` is the safe way to add a pattern, and `ignore()` must follow `unique()`, since it rewrites the last `unique` rule and throws otherwise. How the rules run, and the ones that catch people out, are on [Forms](/docs/authoring/forms#validation).
