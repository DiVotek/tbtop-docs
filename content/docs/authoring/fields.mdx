...
How spans interact with a grid's breakpoints, and how layout blocks take the same placement, is on [Grid columns and columnSpan](/docs/authoring/layout#grid-columns-and-columnspan).
...
If the model already keeps its images in spatie/laravel-medialibrary collections, the separate [Media library](/docs/packages/media-library) package adds a picker bound to one record's collection.
